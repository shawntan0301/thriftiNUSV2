...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Request to unban user:", { adminUserId: userId, targetUserId });
    ...
    // Unban the user using Clerk's API
    const unbanResponse = await fetch(`${CLERK_API_BASE}/users/${targetUserId}/unban`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.CLERK_SECRET_KEY}`,
        'Content-Type': 'application/json',
    ...
    if (!unbanResponse.ok) {
      const errorData = await unbanResponse.text();
      console.error("Failed to unban user:", {
        status: unbanResponse.status,
        statusText: unbanResponse.statusText,
        error: errorData
      });
      throw new Error(`Failed to unban user: ${unbanResponse.statusText}`);
    }

    const result = await unbanResponse.json();
    console.log("Successfully unbanned user:", result);
